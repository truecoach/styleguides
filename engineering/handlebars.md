...
It can be helpful to have a multi-line expression be on a separate line than its parent HTML, particularly when merging in `git`. When needed, you can optionally use [whitespace control](https://handlebarsjs.com/guide/expressions.html#whitespace-control) to remove whitespace before or after an expression.
